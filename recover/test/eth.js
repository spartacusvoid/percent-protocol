...
describe("CEther", function() {
  it("Should deploy CEther", async function() {
    ...
    const old_pETHaddress = addresses.frozenTokens[0].address;
    const old_pETH = new ethers.Contract(old_pETHaddress, abi.OLD_PETH_ABI, signers[0])
...
describe("Recovery", function() {
  xit("Should create new Comptroller and add new pETH contract", async function () {
    const signers = await ethers.getSigners();
